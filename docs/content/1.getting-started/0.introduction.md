...
**Comark** stands for [Co]{.text-primary}mponents in [Mark]{.text-primary}down. It's a syntax extension that lets you use components directly inside your Markdown content:
...
<template>
  <div class="alert alert-warning">This is an important message!</div>
</template>
```
...
export const Alert = ({ type }: Props) => {
  return <div className={`alert alert-${type}`}>This is an important message!</div>
}
```
::
...
```html
<h1>Welcome to m blog</h1>

<p>This is regular <strong>markdown</strong> with a custom component:</p>
