...
The security plugin is included in the `essentials` export and is available by default when using Comark.
...
### `comarkSecurity(options): ComarkPlugin`
...
const userComment = await parse(userInput, {
  plugins: [comarkSecurity({ drop: ['script', 'iframe'] })]
})
...
async function saveArticle(content: string) {
  const sanitized = await parse(content, {
    plugins: [comarkSecurity({ drop: ['script'] })]
  })
