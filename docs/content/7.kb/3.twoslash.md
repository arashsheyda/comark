...
It is implemented as a [Shiki transformer](https://shiki.style/guide/transformers) and wires into Comark through the [highlight plugin](/plugins/core/highlight).
...
const transformer = createTransformerFactory(twoslash.runSync)({
  explicitTrigger: true, // only compile blocks with `twoslash` in their meta string
...
> **Tip** — `explicitTrigger: true` limits compilation to code blocks tagged with `twoslash` in their meta string — plain `ts` blocks are left untouched. Pair with `@shikijs/twoslash/style-rich.css` for the default popup styles.
...
- [Syntax Highlighting](/plugins/core/highlight) — `transformers` option and all highlight settings
- [Shiki Transformers](https://shiki.style/guide/transformers) — full transformer API
- [Twoslash Documentation](https://twoslash.netlify.app) — annotation reference
