...
## Block Components with Named Slots

Named slots use the \`#slot-name\` syntax inside a component block.
...
## Slot Unwrap — \`<slot unwrap="div" />\`

\`ProseUnwrapDiv\` renders \`<slot unwrap="div" />\`.
The wrapping \`div\` is stripped, and its children are rendered directly:

::prose-unwrap-div
:::div{style="border: 1px dashed currentColor; padding: 0.5rem; border-radius: 4px;"}
This content is inside a \`div\` with inline styles — but the \`div\` itself is removed by the unwrap slot.
:::
::
